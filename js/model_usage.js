...
const metricsStyles = `
    ...
    .toolbag-metrics-unload { width: 100%; border: 1px solid #3d8fbd; border-radius: 7px; padding: 8px 10px; cursor: pointer; color: #ccecff; background: rgb(61 143 189 / 15%); font-weight: 650; }
    .toolbag-metrics-unload:hover:not(:disabled) { color: white; background: #287da9; }
    .toolbag-metrics-unload:disabled { cursor: wait; opacity: .55; }
...
const createMetricsPanel = (root, signal) => {
    ...
    const render = () => {
        ...
        const memorySection = addSection("模型显存");
        const memoryCard = createElement("div", "toolbag-metrics-card");
        memoryCard.append(createElement(
            "div",
            "toolbag-metrics-service-description",
            "临时卸载 ComfyUI 与 Ollama 当前驻留的模型并释放缓存，不删除模型文件；下次使用时会自动重新加载。",
        ));
        const unload = createElement(
            "button",
            "toolbag-metrics-unload",
            state.unloadingModels ? "正在释放模型显存…" : "一键临时卸载其他模型",
        );
        unload.type = "button";
        unload.disabled = state.unloadingModels || state.restarting;
        unload.setAttribute("aria-label", "临时卸载 ComfyUI 和 Ollama 已加载模型");
        unload.addEventListener("click", async () => {
            const confirmed = window.confirm(
                "确定要临时卸载当前驻留的模型吗？\n\n不会删除模型文件。正在执行的 ComfyUI 工作流会在安全时机释放；Ollama 已加载模型会立即卸载。",
            );
            if (!confirmed) return;

            state.unloadingModels = true;
            state.unloadMessage = "正在请求 ComfyUI 与 Ollama 释放模型显存…";
            state.unloadMessageType = "";
            render();
            try {
                const response = await api.fetchApi("/toolbag/system/unload-models", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ confirm: "UNLOAD_RUNTIME_MODELS" }),
                    signal,
                });
                const result = await response.json();
                if (!response.ok) {
                    throw new Error(result.error || `释放失败 (${response.status})`);
                }
                const ollama = result.ollama ?? {};
                if (!ollama.available) {
                    state.unloadMessage = "ComfyUI 已安排释放；未连接到 Ollama";
                    state.unloadMessageType = ollama.errors?.length ? "error" : "success";
                } else if (ollama.errors?.length) {
                    state.unloadMessage = `ComfyUI 已安排释放；Ollama 已卸载 ${ollama.unloaded?.length ?? 0} 个，部分失败`;
                    state.unloadMessageType = "error";
                } else {
                    state.unloadMessage = `ComfyUI 已安排释放；Ollama 已卸载 ${ollama.unloaded?.length ?? 0} 个模型`;
                    state.unloadMessageType = "success";
                }
                window.setTimeout(() => load(), 1200);
            } catch (error) {
                if (error.name === "AbortError") return;
                state.unloadMessage = error.message || "释放模型显存失败";
                state.unloadMessageType = "error";
            } finally {
                state.unloadingModels = false;
                render();
            }
        }, { signal });
        memoryCard.append(unload);
        if (state.unloadMessage) {
            memoryCard.append(createElement(
                "div",
                `toolbag-metrics-service-message ${state.unloadMessageType}`.trim(),
                state.unloadMessage,
            ));
        }
        memorySection.append(memoryCard);
    ...
    const load = async () => {
        if (state.loading || state.unloadingModels || state.restarting || signal.aborted) return;
        state.loading = true;
        state.error = "";
        render();
        try {
