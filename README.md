...
侧栏中的 `缺失模型下载` 会汇总浏览器中所有已打开工作流的全部节点引用、工作流归属，以及本机尚未安装的模型。`一键高速下载全部` 会将可直接下载的模型加入统一队列：同一时间只让一个模型独占最多 8 路分片带宽，其余模型按顺序等待，避免多个大文件互相争抢带宽。每个模型都可以单独开始、暂停、继续或取消。
...
侧栏中的 `服务器资源监控` 会实时显示 CPU、内存、显存、温度和硬盘空间。面板最下方提供带二次确认的 `重启 ComfyUI` 按钮；该功能仅在 ComfyUI 由 systemd 管理并配置自动重启时启用，重启会中断当前执行和加载任务。
